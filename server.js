...
Server.REQUESTS = {
	...
	16: function(bufferlist) {
		var ret = Binary(bufferlist)
			.getWord16be('startAddress')
			.getWord16be('registerCount')
			.getWord8('byteCount')
			.getBuffer('data', 'byteCount')
			.end().vars;

		ret.values = [];
		for (var i = 0 ; i < ret.registerCount ; i ++) {
			ret.values.push(ret.data[2*i] * 256 + ret.data[2*i+1]);
		}

		delete ret.byteCount;
		delete ret.data;
		return ret;
	},
...
var genericWriter = function(registers) {
...
Server.RESPONSES = {
	...
	16: function(ret) {
		if (!Array.isArray(ret) || ret.length != 2) {
			throw new Error('Expected to write an "Array" of length 2, first element: start of address, second element: total registers');
		}
		return Put().word16be(ret[0]).word16be(ret[1]).buffer();
	}
};
